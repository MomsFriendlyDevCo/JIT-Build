			...
			paths.map(path => {
				...
				return Promise.resolve()
					...
					.then(()=> JITBuild({
						...
						outdir: fsPath.dirname(session.destPath),
						outfile: fsPath.parse(session.destPath).filename,

						// Pass through options
						minify: settings.minify,
					}))
					.then(()=> fs.utimes(session.destPath, session.sourceStats.atime, session.sourceStats.mtime)) // Set dest file modified time so next hashMatches collides
					.catch(e => {
						if (e === 'STOP') return;
						throw e;
					})
